...
from apps.gpuaas.app.schemas.job import JobCreate
...
class JobService:
    ...
    async def create_job(self, data: JobCreate) -> GPUJob:
        ...
        if data.gpu_count > allocation.gpu_count:
            raise JobCapacityError(
                f"Job requested {data.gpu_count} GPUs but allocation "
                f"contains only {allocation.gpu_count}"
            )
    ...
    async def get_job(self, job_id: UUID) -> GPUJob:
